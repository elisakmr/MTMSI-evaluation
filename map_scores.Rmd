...
png(file.path("carte","map_gamma.jpg"), 
    width = 10, height = 10, units = "in", res=300)

ggplot() +
  geom_sf(data=worldmap)+
  geom_sf(data=shp_gamma, aes(fill = valeur), colour=NA)+
  scale_fill_gradientn(colors=c("#195F24", "#9FE67B", "yellow", "#EC8B3D", "red"), 
                       values = rescale(c(0.5,0.8,1.1,1.4,1.7,2)), name="KGE gamma", limits=c(0.5,2), 
                       breaks=c(0.5,0.8,1.1,1.4,1.7,2), 
                       guide = guide_colorbar(title.position="top", title.hjust=0.5,
                                              barwidth = 10, barheight = 0.5))+
  coord_sf(xlim = c(0, 32), ylim = c(42, 71))+ 
  theme_light()+
