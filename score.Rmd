...
for (i in c(1:length(nuts_hist))){ # loop on mtmsi nuts3 
  ...
  if (length(obs_row)>=1){ # if there is at least one station in the nut
      ...
      if(length(select_altitude)>=1){ # if there is at least one station at same altitude
        ...
        if(max(annee_ok)>=seuil_year){ # if at least one station has enough annual max
          ...
          rmse[[j]] <- apply(obs_nafilt, 1, function(x) 
            rmse(x[which(!is.na(x))], df_hist$value[which(!is.na(x))]))
          rmse_rel[[j]] <- apply(obs_nafilt, 1, function(x) 
            rmse(x[which(!is.na(x))], df_hist$value[which(!is.na(x))])/mean(x, na.rm = TRUE)*100)
          biais[[j]] <- apply(obs_nafilt, 1, function(x) 
            -bias(x[which(!is.na(x))], df_hist$value[which(!is.na(x))]))
          biais_rel[[j]] <- apply(obs_nafilt, 1, function(x) 
            -bias(x[which(!is.na(x))], df_hist$value[which(!is.na(x))])/mean(x, na.rm = TRUE)*100)
          ...
          kgegamma[[j]] <- apply(obs_nafilt, 1, function(x) 
            KGE(x[which(!is.na(x))], df_hist$value[which(!is.na(x))], method = "2012", out.type="full")$KGE.elements[3])
   ...
   rmse_list[[i]]<-rmse
   rmse_rel_list[[i]]<-rmse_rel
   bias_list[[i]]<-biais
   bias_rel_list[[i]]<-biais_rel
   mae_list[[i]]<-mae
   cor_list[[i]]<-corr
   kge_list[[i]]<-kkge 
   kge_gamma_list[[i]] <- kgegamma
...
save(rmse_rel_list, file=file.path("mtmsi_hist", "score", "rmse_rel_list.RData"))
save(bias_list, file=file.path("mtmsi_hist", "score", "bias_list.RData"))
save(bias_rel_list, file=file.path("mtmsi_hist", "score", "bias_rel_list.RData"))
...
save(kge_gamma_list, file=file.path("mtmsi_hist", "score", "kge_gamma_list.RData"))
