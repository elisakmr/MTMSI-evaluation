...
for (lati in c(1:3)){
  ...
  geom_sf(data=sph_bias_list[[lati]], aes(fill = valeur), colour=NA)+
...
for (lati in c(1:3)){
  ...
  pBIASrel[[lati]] <- ggplot() +
  geom_sf(data=worldmap)+
  geom_sf(data=sph_biasrel_list[[lati]], aes(fill = valeur), colour=NA)+
  scale_fill_gradient2(low="red", high="blue", mid="white", midpoint=0, 
                       name="Bias (%)", lim=c(-100,100),#breaks=c(-0.8,-0.5,0,0.5,0.8),
                       guide = guide_colorbar(title.position="top", title.hjust=0.5, 
                                              barwidth = 10, barheight = 0.5))+
  coord_sf(xlim = c(0, 32), ylim = c(44, 71))+ 
  theme_light()+
...
for (lati in c(1:3)){
  ...
  pCOR[[lati]] <- ggplot() +
  geom_sf(data=worldmap)+
  geom_sf(data=sph_cor_list[[lati]], aes(fill = valeur), colour=NA)+ #
...
ggarrange(combi_biasrel, combi_bias, combi_cor, ncol = 3)
dev.off()
