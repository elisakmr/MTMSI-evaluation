...
## File preparation
we set up bbox for latitude discrimination
...
latitude_string = c("> 55°N", "48°N - 55°N", "< 48°N")
...
for (lati in c(1:3)){
  ...
  pBIAS[[lati]] <- ggplot(df_bias %>% filter(lat==get("lati"))) + 
    geom_boxplot(aes(x=factor(alti500),y=valeur, group=alti500, fill=alti500), show.legend = TRUE, 
               alpha=0.5, color="#737373") +
    scale_fill_manual(breaks=c("0-500","500-1000","1000-1500","1500-2000","2000-2500","2500-3000","3000-3500"), 
                    values=palette_alti)+
    theme_classic()+
    labs(x="", y=latitude_string[lati], title="", fill="Elevation (m)") + # Bias (m)
    theme(axis.title.y=element_text(angle = 90, vjust = -135, hjust=0.5, size=10), 
        axis.title.x=element_text(angle = 0, hjust = 1, size=10),
        title = element_text(size=12),
        axis.text.y=element_text(size=9),
        axis.text.x=element_blank(),
...
ggsave(file=file.path("plot", "box_score20-2.jpg"), dpi = 300, width = 11, height = 8)
